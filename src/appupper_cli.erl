...
show_version() ->
    application:load(appupper),
    {ok, Vsn} = application:get_key(appupper, vsn),
    io:format("~s\n",[Vsn]).

show_usage() ->
    getopt:usage(opt_specs(), "appupper", "<options..>"),
    io:format("Example: appupper -n myrelease -p _rel/myrelease -u 1.0 -v 2.0\n\n").
